    ...
    let upload = multer({
        ...
        fileFilter(req, file, cb){
            if (!file.originalname.match(/\.(png|jpeg|jpg|mp4|mp3|mkv)$/)) { 
                req.fileValidationError = "Only support these media formats: PNG, JPEG, JPG, MP4, MP3, MKV";
                return cb(null, false, new Error('Only support these media formats: PNG, JPEG, JPG, MP4, MP3, MKV'));
              }
            cb(undefined, true);
        }
    }).array('file-upload');
    ...
    upload(req, res, err => {
        ...
        function generateThumbName(length){
            ...
            for (let index = 0; index < length; index++) {
                hasil += karakter.charAt(Math.floor(Math.random() * 
                karakterget));
            }
            return hasil;
        }
        ...
        for (let index = 0; index < files.length; index++) {
            if(files[index].mimetype.match(/(mp4|mkv)/)){
                ...
                new ffmpeg(req.files[index].path)
                .takeScreenshots(
                    {
