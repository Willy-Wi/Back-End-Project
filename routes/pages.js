...
const { follow } = require("../controllers/userCon");
const {
    createPost,
    createComment,
    createReport,
...
router.get("/users/:id", loginRequired, async (req, res) => {
    ...
    (SELECT COUNT(*) FROM following where following.user_id = users.user_id) AS Followers, (SELECT COUNT(*) FROM likes where likes.user_id = users.user_id) AS Likes, (SELECT COUNT(*) FROM POSTS where posts.user_id = users.user_id) AS Posts FROM users WHERE Users.user_id = ${req.params.id} GROUP BY Users.user_id;`;
...
router.get("/post/:id/report", async (req, res) => {
    const postId = req.params.id;
...
router.get("/users/:id/edit", isLoggedIn, async (req, res) => {
    let sql = `SELECT name, username, user_id, profile_image, email FROM users WHERE user_id = '${req.params.id}'`;
    let user = await query(sql);
...
router.post("/users/:id/profile/edit", isLoggedIn, addprofile);
router.post("/createpost", isLoggedIn, createPost);
router.post("/createalbum", isLoggedIn, createAlbum);
router.post("/user/:id/report", isLoggedIn, createReport);
router.post("/post/:id2/report", isLoggedIn, createReport);
